...
spa.chat = (function () {
	...
	onTapToggle = function ( event ){
		var temp = (stateMap.position_type == 'closed' ? 'opened' : 'closed');
		setSliderPosition( temp );
		stateMap.position_type = temp;
	};
	...
	onListchange = function ( event) {
		var list_html = String(),
			peopleDb = configMap.people_model.get_db(),
		...
		if ( !list_html ) {
			list_html = '<div class="spa-chat-list-note">To chat alone is the fate of great souls...<br>No one is online.</div>';
			clearChat();
		}
	...
	onUpdatechat = function ( event, msg_map ) {
		...
		writeChat( sender.name, msg_txt, is_user );

		if ( is_user ) {
			jqueryMap.$input.val('').focus();
		}
	};
	onLogin = function ( event, login_user ) {
		console.log('chat onLogin:', login_user);
